...
| Risk | The folded-in render-helper collapse contradicted the letter of `issue-46`'s "one function per section" requirement | `delivery-board (issue-46): Sections sharing a helper each decide their own presence` | ✅ Covered — resolved by rewording `issue-46`'s requirement to permit a shared parameterized helper; see `overrides.md` |
| Risk | The `issue-46` reword couples the two changes, so archiving `issue-46` from a tree without this branch restores the old wording | — | ⚠️ Excluded — an archive-sequencing constraint, not a behavior of the board; both changes are queued for the same bulk archive pass and `overrides.md` states the constraint |
