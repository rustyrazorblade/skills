...
- **Kotlin** — if the project is Kotlin (a `build.gradle.kts`/`settings.gradle.kts`, or `.kt` sources), the deep guidance lives in the standalone **`dev-skills`** plugin, not here. If a developer agent is configured (see `SPEC_FLOW_DEVELOPER_AGENT` in `docs/workflow.md`), you are not the agent running this work. If you are running it anyway, apply the core rules: non-null types by default and **never `!!`**; `when` over `if`/`else` ladders; sealed hierarchies with an exhaustive `when`, so illegal states are unrepresentable; immutability (`val`, read-only collections) with invariants checked in `init`; structured concurrency, never a bare `GlobalScope`; explicit visibility on API surface; the project's logging framework, never `println`. For the full guide, tell the owner to install `dev-skills` and set `SPEC_FLOW_DEVELOPER_AGENT=kotlin-dev`.

This plugin bundles no language style guide. Each one lives in `dev-skills`, beside the developer agent that owns it. Apply the core rules above for the project in front of you, and point the owner at `dev-skills` for the rest.
