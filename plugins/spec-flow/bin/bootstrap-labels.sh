...
# Backlog item filed by /tech-debt (review-tools), and the closed marker issue each of its runs logs
# (the durable timestamp project-manager reads to compute the once-a-week/20-merges cadence).
gh label create "type:tech-debt"    --color fef2c0 --description "Structural improvement filed by /tech-debt (review-tools) — SOLID, duplication, or layering" --force || true
gh label create "tech-debt-review"  --color ededed --description "Marks a closed log issue for a completed /tech-debt (review-tools) run — audit trail, not a work item" --force || true
