...
**The two seams stay yours.** The scheduler changes only *dispatch*, never approval. It parks
data-model work at Seam 1, honors `merge-on-green`, and otherwise the `issue-manager` holds for you at
both seams — exactly as if you had dispatched the issue yourself. You still attach to each
`issue-manager` to resolve its seams.
...
| `/spec-flow:scheduler` | foreground / `/loop` | Optional churn mode for `project-manager`: pull the highest-priority `status:ready` issue, claim it, and spawn an `issue-manager`, up to 3 in flight. Changes dispatch only — parks data-model at Seam 1, honors `merge-on-green`, and otherwise holds both seams for you. Session-driven; drive it by hand or via `/loop`. See **Scheduler** above. |
